...
prompt="Use pos-magents as the independent PM. Read docs/agents/status.md and the tracked contracts. Process exactly one coordination cycle for this event, commit the decision, dispatch only dependency-ready work, then stop. Event: $event_json"
if codex exec resume "${registration[1]}" "$prompt" >>"$runtime/pm-scheduler.log" 2>&1; then
