...
prompt="Use pos-agent-worker. Execute only $task_id from committed contract $task_path at coordination commit $contract_commit. Work only in $worktree on its registered branch. Run every contract gate to normal exit, write and push the required report, notify PM, then stop. Do not approve, merge, select another task, or upgrade model/reasoning."
