...
def parser() -> argparse.ArgumentParser:
    ...
    artifact_show.add_argument("--project", required=True)
    ...
    pipeline_run.add_argument("--project", required=True)
    ...
    pipeline_resume.add_argument("--project", required=True)
    ...
    stage_run.add_argument("--project", required=True)
    ...
    stage_retry.add_argument("--project", required=True)
...
def _stream_events_jsonl(commands: MountainCommands, project_id: str, run_id: str, after: int = 0) -> None:
    """Stream events as JSONL to stdout."""
    cursor = after
    while True:
        result = commands.list_events(project_id, run_id, cursor)
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (args.resource, args.action) == ("project", "create"):
        ...
        return commands.create_project(title, request.get("pipeline", args.pipeline), Engine(request.get("engine", args.engine)), request)
    ...
    if (args.resource, args.action) == ("logs", "tail"):
        result = commands.list_logs(args.project, args.run)
        if hasattr(args, "follow") and args.follow:
            ...
            log_path = commands.repository.run_dir(args.project, args.run) / "observability" / "logs.jsonl"
    ...
    if (args.resource, args.action) == ("artifact", "show"):
        return commands.artifact_show(args.project, args.run, args.key)

    # ── pipeline ─────────────────────────────────────────────────────
    if (args.resource, args.action) == ("pipeline", "run"):
        result = commands.pipeline_run(args.project, getattr(args, "run", None), args.policy, args.stage)
    ...
    if (args.resource, args.action) == ("pipeline", "resume"):
        result = commands.pipeline_resume(args.project, getattr(args, "run", None), args.policy)
        if hasattr(args, "events") and args.events == "jsonl":
            pass
        return result
    ...
    if (args.resource, args.action) == ("stage", "run"):
        ...
        if args.stage == "segment-script":
            ...
            return commands.segment_script(args.project, run_id, args.script)
            ...
            return commands.render_visuals(args.project, run_id, renderer)
            ...
            if not run_id:
                raise ValueError("需要 --run 或项目有活跃运行")
            return commands.pipeline_run(args.project, run_id, "targeted", args.stage)
    ...
    if (args.resource, args.action) == ("stage", "retry"):
        ...
        if not run_id:
            raise ValueError("需要 --run 或项目有活跃运行")
        return commands.stage_retry(args.project, run_id, args.stage, args.unit, args.visual)
