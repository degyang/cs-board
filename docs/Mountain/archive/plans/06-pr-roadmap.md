...
> **路线基线，不是当前进度表。** M03 Legacy接入已取消，新产品与Legacy强隔离；M01/M02/M04/M05/M06的核心代码已交付但Skills真实闭环仍未验收；M07正在进行。权威进度见 [23-current-delivery-status.md](../../23-current-delivery-status.md)。后续不按本文件“首个实施动作”重新开始。

## 0. 当前落地状态

| 里程碑 | 当前判断 | 说明 |
|---|---|---|
| M01 | 已完成 | Schema与文档基线已建立。 |
| M02 | 已完成 | Domain、Repository、Artifact、Telemetry和Adapters已进入集成基线。 |
| M03 | 已取消 | 不接入Legacy；旧代码后续归档，只作参考。 |
| M04 | 部分完成 | CLI与7个Skill文件存在；Skill工作单驱动和真实六阶段运行待验收。 |
| M05 | 核心完成、实跑待验收 | Voice Unit、IndexTTS、Whisper/fallback代码存在；需真实Task证据。 |
| M06 | 核心完成、实跑待验收 | Storyboard、插画、渲染、合成存在；Codex出图回存闭环未完成。 |
| M07 | 进行中 | 设置/资产/队列已完成阶段验收；新建任务和execution plan正在纠偏。 |
| M08 | 未开始 | 等标准流程真实成片后再做发布加固。 |
| M09 | 暂缓 | 自定义参考和动态信息图最后处理。 |
