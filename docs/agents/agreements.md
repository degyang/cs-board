...
## CORE-RUNTIME-007-CONDITIONAL-20260902 — 运行时修复有条件通过

- Time: `2026-09-02T20:29:00+08:00`
- Status: `ACTIVE`
- Source: `USER-DIRECTIVE`

### 用户裁决

1. PM 将 `CORE-RUNTIME-007@09009f1` 记为有条件通过，并以标准依赖状态 `APPROVED` 关闭该任务；
   “有条件”必须写入 Review/决策说明，不得解释为最终用户验收通过。
2. 已验证的核心目标是运行时挂起修复：全量 pytest 正常退出，结果为
   `506 passed, 5 skipped, 4 warnings, 3 subtests passed in 96.19s`，且没有遗留测试子进程或端口监听。
3. 5 个既有 skip 与 `de57fab...09009f1` 范围内 25、27 号历史文档的行尾空格登记为遗留问题；
   它们不再阻塞本次 runtime 修复，也不得在未获得对应修改权限时由 CORE 任务越界修正。
4. 权限允许后，PM 应将上述两类遗留问题拆为边界明确的后续治理任务，不得重新打开已解决的运行时挂起问题。
5. `MEDIA-PREFLIGHT-004` 的 runtime-timeout 依赖据此解除；PM 仍须独立处理 Media 自身门禁，且不得提前批准
   `MEDIA-E2E-003` 或代替用户宣布第一阶段验收通过。
