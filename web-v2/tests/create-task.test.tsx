...
describe('CreateTaskPage six-tab preview-first flow', () => {
  ...
  it('moves roving focus and exposes a matching active tabpanel for keyboard navigation', async () => {
    renderPage(); await ready(); const intro = screen.getByRole('tab', { name: '任务介绍' }); fireEvent.keyDown(intro, { key: 'ArrowRight' }); const script = screen.getByRole('tab', { name: /视频文案/ }); expect(script).toHaveFocus(); expect(script).toHaveAttribute('tabindex', '0'); expect(screen.getByRole('tabpanel')).toHaveAttribute('id', 'tab-panel-script'); expect(script).toHaveAttribute('aria-controls', 'tab-panel-script'); fireEvent.keyDown(script, { key: 'End' }); expect(screen.getByRole('tab', { name: '成片设置' })).toHaveFocus(); expect(screen.getByRole('tabpanel')).toHaveAttribute('aria-labelledby', 'tab-final')
  })
