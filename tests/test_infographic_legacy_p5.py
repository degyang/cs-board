...
def test_native_and_legacy_route_inventory_is_explicit() -> None:
    ...
    native = (root / "backend/mountain_server.py").read_text(encoding="utf-8")
    legacy = (root / "backend/mountain_api.py").read_text(encoding="utf-8")
    ...
    assert not any(item == "backend.mountain_api" or item == "csboard.application.legacy_bridge" for item in imports)
...
def test_native_commands_and_remotion_adapter_have_no_legacy_or_fallback_imports() -> None:
    root = Path(__file__).parents[1]
    forbidden = {"backend.server", "backend.mountain_api", "backend.mountain_stages", "csboard.application.legacy_bridge"}
...
"""
    ...
    assert json.loads(result.stdout) == {"backend.server": False, "backend.mountain_api": False,
                                         "backend.mountain_stages": False, "csboard.application.legacy_bridge": False}
