...
class FoundationTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.project_id = new_id("project")
        ...
        self.repository.create_project(self.project)
    ...
    def test_project_and_run_are_persisted_with_correlation(self) -> None:
        stored_project = self.repository.get_project(self.project_id)
        stored_run = self.repository.get_run(self.project_id, self.run_id)
        ...
        self.assertTrue((self.repository.run_dir(self.project_id, self.run_id) / "artifacts" / "index.json").exists())
    ...
    def test_artifact_commit_is_atomic_and_invalidates_only_downstream(self) -> None:
        ...
        plan = b'{"voice_units": []}'
        store.commit_bytes(self.project_id, self.run_id, "planning.av-plan", "plans/av-plan.json", plan, "plan")
        store.commit_bytes(self.project_id, self.run_id, "audio.voice-manifest", "audio/voice.json", b"{}", "voice")
        store.commit_bytes(self.project_id, self.run_id, "timing.timeline", "timing/timeline.json", b"{}", "timing")

        invalidated = store.invalidate_from(self.project_id, self.run_id, "planning.av-plan", "plan changed")
        ref = store.get(self.project_id, self.run_id, "planning.av-plan")
        timeline = store.get(self.project_id, self.run_id, "timing.timeline")
        target = self.repository.run_dir(self.project_id, self.run_id) / "artifacts" / "plans" / "av-plan.json"
    ...
    def test_artifact_paths_cannot_escape_project(self) -> None:
        ...
        for path in ("../escape.json", "C:\\escape.json", "nested\\escape.json"):
            with self.subTest(path=path), self.assertRaises(DomainError) as context:
                store.commit_bytes(self.project_id, self.run_id, "bad", path, b"bad", "test")
    ...
    def test_jsonl_telemetry_is_ordered_and_redacted(self) -> None:
        ...
        first = telemetry.append_event(self.project_id, self.run_id, {"event_type": "RunStarted"})
        second = telemetry.append_event(self.project_id, self.run_id, {"event_type": "StageStarted"})
        telemetry.append_log(self.project_id, self.run_id, {"level": "info", "api_key": "canary-secret-123"})
        telemetry.append_audit(self.project_id, self.run_id, {"action": "run.start", "authorization": "Bearer canary-secret-123"})

        events = telemetry.read_events(self.project_id, self.run_id, after_sequence=1)
        logs = (self.repository.run_dir(self.project_id, self.run_id) / "observability" / "logs.jsonl").read_text(encoding="utf-8")
    ...
    def test_diagnostic_bundle_excludes_media_and_redacts_data(self) -> None:
        telemetry = JsonlTelemetry(self.repository)
        telemetry.append_log(self.project_id, self.run_id, {"token": "canary-secret-123"})
        media = self.repository.run_dir(self.project_id, self.run_id) / "media" / "source.wav"
        media.write_bytes(b"private-audio")

        bundle = telemetry.export_diagnostic_bundle(self.project_id, self.run_id)
